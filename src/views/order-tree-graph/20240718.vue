...
export default {
  ...
  methods: {
    ...
    rootDataHandle(rootData) {
      ...
      // 获取树的深度
      this.deep = this.getTreeDeep(data);
    ...
    setOptions() {
      ...
      this.options = {
        ...
        layout: {
          ...
          getWidth: () => {
            // 半动态宽度--树的extend为3，固定值300差不多，但是树的extend为4时，部分边得箭头有点歪
            // 经测试，*40比较合适
          ...
          // },
          // getHGap: () => {
          //   return this.deep > 5 ? this.deep * 10 : 80
          // }
        },
      };
    },
    // 树图渲染完成后的回调--目前用于设置树图的宽高
    afterLayout() {
      // 获取树图实例
      const graph = this.$refs.treeRef.graphInstance;
      // 设置树图的宽度--根据树的深度设置（树：左----》右）
      const width =
        this.deep * this.maxTreeOptionWidth > 600
          ? this.deep * this.maxTreeOptionWidth
          : 600;
      //  设置树图的高度--根据树的广度设置（树：左----》右）
      const height =
        this.maxChildrenLen * 150 > 600 ? this.maxChildrenLen * 150 : 600;
      console.log("width:", width, "height:", height, this.maxChildrenLen);
      // 设置树图的宽/高度
    ...
    getTreeDeep(treeData) {
      ...
      while (arr.length) {
        ...
        for (let i = 0; i < arr.length; i++) {
          if (arr[i].children) {
            temp = temp.concat(arr[i].children);
          }
        }
        if (temp.length) {
          deep++;
